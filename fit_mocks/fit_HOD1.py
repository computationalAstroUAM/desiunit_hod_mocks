...
def CalcNcent(M, mu, sig):
        return 0.5*(1.0+special.erf((M-mu)/sig))
...
#b_targ /= np.sqrt(0.94806042)  ## corrected 
#b_targ /= np.sqrt(0.91049307)  ## corrected twice (f=0.30)
#b_targ /= np.sqrt(0.975)  ## corrected thrice (f=0.30)
...
#    	out = np.append(out,np.array([[fsat,mu_bf,ac_bf,as_bf]])) 
    ...
    if verbose:
        print('Re-scaled params:  logM1= {:10.4f}'.format(m1_bf))
        print('  ')
    if verbose:
        print('-----Test-----')
...
if (times7):
	if (corr==0):
		np.savetxt('HOD1_x7.params',out,fmt='%.2f %.6f %e %e')
	else:
		np.savetxt('HOD1_erfc_PL_corr_x7.params',out,fmt='%.2f %.6f %e %e')
else:
	if (corr==0):
		np.savetxt('HOD1.params',out,fmt='%.2f %.6f %e %e')
	else:
		np.savetxt('HOD1_corr.params',out,fmt='%.2f %.6f %e %e')
